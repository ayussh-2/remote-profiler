...
const COLS = ["Time", "Type", "Severity", "Area m2", "Depth m", "Vol L", "Conf", "Lat", "Lng"];
    ...
    return (
        <div className="logs-wrapper">
            <div className="logs-toolbar">
                ...
                <button onClick={onRefresh} title="Refresh" className="refresh-btn">
                    <RefreshCw size={12} />
                    REFRESH
                </button>
            </div>
            ...
            {logs.length === 0 ? (
                ...
                <div className="logs-scroll">
                    <table className="data-table">
                        ...
                        <tbody>
                            {logs.map((log, i) => (
                                <tr key={log.id || i} className={i % 2 === 0 ? "row-alt" : ""}>
                                    <td>{log.datetime || log.timestamp}</td>
                                    <td>
                                        <span className="badge badge-dim">
                                            {(log.defect_type || "pothole").toUpperCase()}
                                        </span>
                                    </td>
                                    <td>
                                        <span className={`badge ${SEVERITY_CLS[log.severity] || "badge-accent"}`}>
                                            {log.severity || "--"}
                                        </span>
                                    </td>
                                    <td>{log.area_m2}</td>
                                    <td>{log.depth_m}</td>
                                    <td>
                                        <span className="badge badge-accent">{log.volume_liters} L</span>
                                    </td>
                                    <td>
                                        <span className={confBadge(log.confidence)}>
